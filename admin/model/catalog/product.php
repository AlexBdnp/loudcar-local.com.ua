...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		LEFT JOIN " . DB_PREFIX . "product_to_category poc ON (p.product_id = poc.product_id) 
		LEFT JOIN " . DB_PREFIX . "category_description cd ON (poc.category_id = cd.category_id) 
		LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)
		WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		
		
		if (!empty($data['filter_category'])) {
			$x = explode(";  ", $data['filter_category'])[count(explode(";  ", $data['filter_category'])) - 1];
			$sql .= " AND cd.name LIKE '" . $this->db->escape($x) . "%'";
		}
