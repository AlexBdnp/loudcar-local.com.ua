...
class ControllerCatalogProduct extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			if (isset($this->request->get['filter_category'])) {
				foreach(explode("&", str_replace("&gt;", ">", $_SERVER['REQUEST_URI'])) as $chunk) {
					$param = explode("=", $chunk);
					if ($param[0] == "filter_category") $url .= '&filter_category=' . $param[1];
				}
			}
	...
	protected function getList() {
		...
		$data['sort_category'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.category' . $url, true);
		$data['sort_manufacturer'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.manufacturer' . $url, true);
